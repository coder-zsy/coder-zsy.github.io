...
**本篇内在逻辑**：本部分建立全书的**机制基础**——人脑存在两种加工方式（系统 1 / 系统 2）。论证顺序：先界定两系统的分工与冲突（Ch.1）→ 再说明注意力的有限性如何导致「看不见」与惰性（Ch.2–3）→ 再展示系统 1 的联想、启动与流畅性如何悄悄左右判断（Ch.4–5）→ 最后落到情境依赖与「用简单问题替代难题」的启发式（Ch.6–9）。整体是从「谁在干活」到「何以出错」的递进。

### 第 1 章 一张愤怒的脸和一道乘法题

**【论证线索】**  
从日常现象（脸孔识别 vs 心算）切入，引出两套加工方式的存在 → 用瞳孔实验提供生理证据 → 得到「系统 1 自动、系统 2 费力、二者可冲突」的结论；本章是全书双系统概念的起点。

**【核心议题】**  
系统 1 负责自动加工（如识别人脸、理解句子），系统 2 负责需要集中注意力的任务（如复杂运算）；当系统 1 遇到困难时会求助系统 2，二者存在冲突与自我控制。

**【关键证据/案例】**  
看到愤怒的脸会立刻产生情绪反应（系统 1）；做 17×24 这类乘法需要心算与工作记忆（系统 2）。**瞳孔与心算**：让被试做不同难度的乘法（如 2+2 vs 17×24），瞳孔直径随题目难度增大而明显扩张，可作为「系统 2 被激活」的客观生理指标——认知越费力，瞳孔扩张越明显。
...
**【关键证据/案例】**  
「香蕉」「呕吐」的联想会让人对后续信息更敏感。**铅笔与微笑**：让被试用嘴含住铅笔（被迫做出类似微笑的肌肉动作）时，会觉得漫画更好笑；若用嘴唇含住笔尖（类似噘嘴）则效果相反——身体状态通过联想无意识地影响情绪与判断，即启动效应与具身认知。
...
**本篇内在逻辑**：在第一部分已建立「双系统 + 替代与启发式」的基础上，本部分**逐类展开判断中的系统性偏误**。论证顺序：先谈小样本与随机性的误读（Ch.10）→ 锚定与可得性（Ch.11–12）→ 风险感知与政策（Ch.13）→ 代表性、基础比率与合取谬误（Ch.14–16）→ 回归均值及对直觉预测的约束（Ch.17–18）。整体是从「统计思维缺失」到「如何用基准与回归纠偏」的递进，并为第三部分「过度自信与预测」铺路。
...
**【关键证据/案例】**  
**小样本的错觉**：统计规律只在大样本中稳定，小样本波动极大（如抛硬币连续几次正面），出错风险可能高达 50%；人们却常对小样本过度解读、赋予因果意义，甚至对完全随机的事件编故事——忽视随机性，高估规律性。普遍存在「信任多于质疑」的偏见。
...
**本篇内在逻辑**：在判断偏误（第二部分）与预测纠偏（Ch.18）之后，本部分聚焦**过度自信及其后果与纠偏**。论证顺序：后见之明与「知道」的错觉（Ch.19）→ 预测为何不可靠、公式为何优于专家（Ch.20–21）→ 何时可信专家直觉（Ch.22）→ 外部意见与规划谬误（Ch.23）→ 乐观偏差与事前验尸（Ch.24）。整体是「偏误描述 → 何时信直觉 → 如何用流程纠偏」的递进。
...
**本篇内在逻辑**：从「判断与预测」转入**选择与风险**——人在面对得失、概率时的系统性偏离。论证顺序：理性人假设的失效与前景理论的引入（Ch.25–26）→ 损失厌恶、禀赋效应、参照点与公平（Ch.27–28）→ 概率权重与四重模式（Ch.29–30）→ 宽框架与风险政策（Ch.31）→ 心理账户、评估逆转与框架效应应用（Ch.32–34）。整体是「期望效用失效 → 前景理论核心概念 → 应用与助推」的递进。
...
**本篇内在逻辑**：在判断与选择偏误之后，本部分追问**谁在体验、谁在决策**——体验自我与记忆自我的分裂。论证顺序：决策效用与体验效用的不一致（Ch.35）→ 峰终定律与时长忽视（Ch.36）→ 幸福的双重含义与测量困境（Ch.37–38）。整体是「两个自我 → 记忆如何改写体验 → 幸福与生活满意度的概念澄清」，与全书收束于「人并非单一理性主体」。
...
> 在缺乏有效反馈的环境下，技能无法通过经验可靠习得——可预测性取决于环境结构。

> 你是在为「更好的体验」还是「更好的故事」做选择？体验自我与记忆自我并非同一套逻辑。
